...
echo "With spaces: " . count($transaksiList) . "\n";

$idsTrimmed = array_map('trim', $ids);
$stmtInfo->execute($idsTrimmed);
$transaksiListTrimmed = $stmtInfo->fetchAll(PDO::FETCH_ASSOC);
echo "Trimmed: " . count($transaksiListTrimmed) . "\n";

$qCols = "SHOW COLUMNS FROM dbold.fisiosfjual LIKE 'ID'";
$stmtCols = $conn->query($qCols);
print_r($stmtCols->fetch(PDO::FETCH_ASSOC));
